...
- 算法适用
    - gpt、bert等模型的特点
    - 语言模型的目标，**几乎都是根据当前词预测下一个词**

| 算法名称 | 训练目标 | 优化器 | 参数量 | 结构描述 | 特点 | 优点 | 缺点 | 适用任务 | 是否常用
| :-| :-| :-| :-| :-| :-| :-| :-| :-| :-
| elmo | 前/后向LSTM语言模型的最大似然 | 视预训练任务，一般为Adam和Adadelta优化器 | 94M | 3层双向（前向和后向）lstm + 线性层 | ELMo为一个典型的自回归预训练模型<br>其包括两个独立的单向LSTM实现的单向语言模型进行自回归预训练 | 1. 采用双向（前后向拼接而非bi-lstm）LSTM语言模型来捕获句子更长的依赖关系,提高了模型最终效果；<br>2. 使得word embeding根据输入句子不同而动态变化，**解决一词多义问题** | **使用 LSTM 作为特征提取器，效果不如 Transformer 自注意力模型好；**<br>使用向量拼接方式融合上下文特征，效果不够好；<br>**LSTM 无法并行计算，模型训练效率低。** | QA、文本蕴含、情感分析
| gpt | 无监督语言模型（Pre-training） | Adam优化器 |117M | 12层Transformer的decoder + softmax层 | GPT，一个生成式的预训练模型，也是一个自回归语言模型 | 1. 在sentence level的任务中效果不错，<br> 2. GPT更加适合文本生成类任务 | 仅采用从左到右的transform，即预训练的时候只是利用上文预测下一个单词 | NLI、QA、语义相似度、分类
| bert | MLM的似然函数 + NSP的似然函数 | AdamW | 104M | 12层Transformer的encoder + softmax层 | 基于transformer的双向编码表示，它是一个预训练模型，模型训练时的两个任务是预测句子中被掩盖的词以及判断输入的两个句子是不是上下句。<br>采用字符级别的BPE编码 | mlm的双向Transform进行预训练，可以生成深层的双向语言表征；| masked 15%的方式，使得训练过程与fine-tuning过程的差异性 | 句子对分类任务、分类任务、QA任务、NER等 | 是
| albert | | | 40M | | 减少模型参数量：<br>对嵌入层的参数进行因子分解 <br>层间参数共享 <br> 使用句子顺序预测（SOP）任务代替BERT的下一句预测（NSP）任务
| roberta | | | | | **采用了动态掩码，dynamic Masking的方式**<br>干掉了NSP任务<br>超参优化，更大的batch size和更多的数据量 | 
| xlnet | | |  | | | 通过permute的方式，保证了可以收到上下文信息，且可以保证**训练过程与fine-tuning过程一致** | |  | 是
...
#### RNN vs LSTM vs GRU
- 不同之处
    - RNN：引入了循环的概念，但是在实际过程中却出现了初始信息随时间消失的问题
    - LSTM：因为LSTM有进有出且当前的cell informaton是通过input gate控制之后叠加的，RNN是叠乘，因此LSTM可以防止梯度消失或者爆炸。
        - 推导forget gate，input gate，cell state， hidden information等因为LSTM有进有出且当前的cell informaton是通过input gate控制之后叠加的，
        - RNN是叠乘，因此LSTM可以防止梯度消失或者爆炸的变化是关键
...
| 模型对比 | 优点 | 缺点
|:- |:- |:-
| CRF | 征灵活，可以容纳较多的上下文信息，能够做到全局最优 | 速度慢
| CRF vs HMM | CRF没有HMM那样严格的独立性假设条件，因而可以容纳任意的上下文信息。特征设计灵活
| CRF vs MEMM | CRF计算全局最优输出节点的条件概率，它还克服了最大熵马尔可夫模型标记偏置（Label-bias）的缺点
| CRF vs ME | CRF是在给定需要标记的观察序列的条件下，使用维特比算法，计算整个标记序列的联合概率分布，<br>而不是在给定当前状态条件下，定义下一个状态的状态分布。
...
- 介绍
    ...
    - 广义上的优点：
        - （1）识别位移、缩放及其他形式扭曲不变性的二维图形 
        - （2）隐式从训练数据中学习特征
        - （3）同一特征映射面的神经元权值相同—–>课并行学习
        - （4）局部权值共享—->在语音识别、图像处理方面有优势、更接近实际生物神经网络、降低网络复杂性、避免特征提取和分类过程中的数据重建复杂度
    - 较一般神经网络在图像处理方面的优点：
        - （1）图像和网络拓扑更好吻合 
        - （2）特征提取和模式分类同时进行，并同时在训练中产生
        - （3）权值共享—–>减少参数、结构简单、适应性更强
    - 应用：语音分析、图像识别等 
...
- 卷积
    ...
    - 池化，max pooling和average pooling相对于全连接， **缩减输入数据的规模**
...
![RNN演变介绍](https://cdn.jsdelivr.net/gh/w666x/image/NLP_base/RNN演变介绍.jpg)
...
- 为什么LSTM模型中既存在Sigmoid又存在Tanh两种激活函数？
    - 二者目的不一样：sigmoid 用在了各种gate上，产生0~1之间的值，这个一般只有sigmoid最直接了。
